...
class Orchestrator {
   ...
   private:
	...
	// device essentials - in our sample, we have ONE "actual" device and ONE shadow representing it in mbed Cloud... 
	void *m_device;					// the "actual" underlying device 
	DeviceShadow *m_device_shadow;			// the shadow of the "actual" device within mbed Cloud (via PT)
};
