...
void Orchestrator::initialize(void *device) {
    ...
    // create our device shadow (only one for the one actual device we have underneath...)
...
bool Orchestrator::initializePT(int argc,char **argv) {
	...
	DocoptArgs args = docopt(argc, argv, /* help */ 1, /* version */ "0.1");
        ...
        this->m_pt_ctx = (protocol_translator_api_ctx_t *)malloc(sizeof(protocol_translator_api_ctx_t));
        if (!args.protocol_translator_name) {
            printf("Missing required options: --protocol-translator-name parameter is mandatory\n");
            return false;
        }
...
void Orchestrator::ptRegisterFailure() {
    this->shutdown();
}
...
void Orchestrator::ptRegisterSuccess() {
    // DEBUG
    printf("Orchestrator: PT connected and registered. Ready for device shadow creation...\n");
...
void Orchestrator::ptIsReady(struct connection *connection) {
    pt_status_t status = pt_register_protocol_translator(connection, &Orchestrator::ptRegisterSuccessCB, &Orchestrator::ptRegisterFailureCB, (void *)this);
...
DeviceShadow *Orchestrator::getDeviceShadow() {
    return this->m_device_shadow;
}

// get the underlying device
void *Orchestrator::getDevice() {
